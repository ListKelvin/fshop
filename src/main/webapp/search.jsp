...
<c:import url="include/Header.jsp"><c:param name="title" value="Search"/></c:import>
    ...
    <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 gap-3 p-5 justify-content-center"> 
        ...
        <c:forEach items="${requestScope.LIST_PRODUCT}" var="products" varStatus="counter">
            <tag:product_card className="col-md-4" idProduct="${products.id}" disabled="${requestScope.user == null ?'disabled': ''}" id="${counter.count}" category="${products.categoryName}" productName="${products.title}"  img="images/${products.image}" price="${products.price}"/>
        </c:forEach>
